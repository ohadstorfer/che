...
import { CLITIC_LEMMAS, OPTIONAL_LEMMAS, SUBJECT_PRONOUNS } from './rules.mjs';
import { split } from './tokenize.mjs';
...
const personalVerb = (t) => t.forms.find((f) => f.pos === 'verb' && f.features?.person)?.features ?? null;
const isClitic = (t) => t.forms.some((f) => f.pos === 'pron' && (CLITIC_LEMMAS.has(f.lemma) || f.features?.clitic));
const subjectPronoun = (t) => t.forms.find((f) => f.pos === 'pron' && SUBJECT_PRONOUNS.has(f.lemma))?.lemma ?? null;
const isOptionalWord = (t) => t.forms.length > 0 && t.forms.every((f) => OPTIONAL_LEMMAS.has(f.lemma));

/** Index of the first token from `i` that isn't a clitic. */
const skipClitics = (tokens, i) => {
  while (i < tokens.length && isClitic(tokens[i])) i++;
  return i;
};

/** A pronoun right before its own verb ("yo soy", "yo me llamo") can go. */
function droppablePronoun(tokens, i) {
...
/**
 * @param tokens   from tokenize(): { surface, core, forms }
 * @param en       the English prompt
 * @param outline  where a variant's swapped-in words come from
 * @returns        alternative Spanish sentences, `es` itself not included
 */
export function generateVariants(tokens, en, outline) {
  const optional = optionalTokens(tokens);
  const persons = tokens.map(personalVerb).filter(Boolean);
  ...
  const pronounFor = (f) => (f.person === 1 ? 'yo' : f.person === 2 ? 'vos' : third);
  ...
  tokens.forEach((t, i) => {
    ...
    const piece = { lead, word: core, tail, name: t.forms.length > 0 && t.forms.every((f) => f.pos === 'propn') };
    ...
    const insert = { original: null, choices: [null] };
  ...
  const seen = new Set([key(slots.map((s) => s.original).filter(Boolean))]);
  const out = [];
  const walk = (k, picks) => {
...
const key = (pieces) => pieces.map((p) => p.word.toLocaleLowerCase('es')).join(' ');
...
const upper = (w) => w.charAt(0).toLocaleUpperCase('es') + w.slice(1);
const lower = (w) => w.charAt(0).toLocaleLowerCase('es') + w.slice(1);
 ...
 */
export function uncoveredTokens(tokens, en, loose = []) {
  const words = englishWords(en);
  const optional = optionalTokens(tokens);
